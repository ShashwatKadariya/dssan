...
@section('content')
    <div class="p-4">
        ...
        <form action="{{ route('fact.store') }}" method="POST" enctype="multipart/form-data" class="w-2/4 m-auto"
            ...
            <div class="relative z-0 w-full mb-6 group">
                <label class="text-sm text-gray-500">Graduated Text</label>
                <textarea rows="2" name="graduated_text"
                    class="block mt-2 p-2 w-full text-sm text-gray-900 rounded bg-gray-50 border border-gray-200 focus:outline-none focus:ring-0 focus:border-blue-600"></textarea>
                @error('graduated_text')
                    <p class="text-sm text-red-500"><small>{{ $message }}</small></p>
                @enderror
            </div>
            <div class="relative z-0 w-full mb-6 group">
                <label class="text-sm text-gray-500">Current Members Text</label>
